...
class TextRecognizer:
    def __init__(self, model_dir=None, device_id: int | None = None):
        ...
        config['weights'] = os.path.join(get_project_base_directory(), "vietocr_assets", "weight", "vgg_seq2seq.pth")

        #transformer
        #config = Cfg.load_config_from_name('vgg_transformer')
        #config['weights'] = os.path.join(get_project_base_directory(), "vietocr_assets", "weight", "vgg_transformer.pth")

        # Avoid runtime downloads in production containers. If you really want it:
        # set `VIETOCR_CNN_PRETRAINED=1`.
        config['cnn']['pretrained'] = os.getenv("VIETOCR_CNN_PRETRAINED", "0").strip().lower() in {"1", "true", "yes"}
        config['device'] = 'cpu'
